...
export class Snake extends Laya.Sprite {
    ...
    constructor(
        ...
        , x: number = Game.ins().gameMainUI.map.width / 2
        , y: number = Game.ins().gameMainUI.map.height / 2
    ) {
        super()
    ...
    loaded(x: number, y: number): void {
        ...
        this.visible = true
        //每个格子10px
        this.bodySpace = Math.floor(this.width / 10 * 8) //32px
        for (let index = 1; index <= this.getBodyNum(); index++) {//5个
